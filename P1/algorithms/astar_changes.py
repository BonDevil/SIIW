...
def astar(start, end, time, heuristic):
    ...
    queue = [(0, start, '')]
    while queue:
        current_f_distance, current_node, curr_line = heapq.heappop(queue)
        if current_node == end:
            return f_distance, came_from

        if graph[current_node] is not None:
            for edge in graph[current_node]:
                (id, company, next_line, departure_time, arrival_time, start_stop, next_node, start_stop_lat, start_stop_lon,
                 end_stop_lat, end_stop_lon) = edge

                # calculating distance based on users criterion
                temp_g_distance = g_distance[current_node]
                if curr_line != next_line:
                    temp_g_distance += 1000
                ...
                if temp_g_distance < g_distance[next_node]:
                    ...
                    f_distance[next_node] = temp_g_distance + heuristic(start_stop_lat, start_stop_lon, end_stop_lat,
                                                                        end_stop_lon)
                    came_from[next_node] = current_node + ", " + next_line + ", " + departure_time + \
                                           ", " + next_node + ", " + arrival_time
                    heapq.heappush(queue, (f_distance[next_node], next_node, next_line))
